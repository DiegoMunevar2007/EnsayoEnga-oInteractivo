...
import PhotographDissolve from '../interactive/PhotographDissolve'
  ...
  return (
    <SceneShell scene={scene} index={index}>
      <div className="w-full h-full flex flex-col lg:flex-row items-center justify-center px-6 sm:px-10 lg:px-16 py-16 sm:py-20 gap-8 lg:gap-16 xl:gap-24">
        ...
        {/* Right: PhotographDissolve */}
        <motion.div
          initial={{ scale: 0.95 }}
          animate={{ scale: 1 }}
          transition={{ duration: 0.6, delay: 0.5, ease: [0.22, 1, 0.36, 1] }}
          className="w-full lg:w-[45%] h-full min-h-[400px]"
        >
          <PhotographDissolve sentence={finalSentence} />
        </motion.div>
      </div>
    </SceneShell>
  )
